...
public:
    ...
    ~ofxLedGrabLine() {
        ofLogVerbose("[ofxLedGrabLine] Detor: clear points");
        points.clear();
    };
...
public:
    ...
    ~ofxLedGrabCircle() {
        ofLogVerbose("[ofxLedGrabCircle] Detor: clear points");
        points.clear();
    };
    ...
    void updatePoints() {
        ...
        for (int i=0; i<pixelsInLine; i++) {
            ...
            if (tmp.x >= 0 && tmp.y >= 0) points.push_back(tmp);
